...
export const AddStudyForm = () => {
    ...
    const onSubmit = async (data: {
        ...
        try {
            ...
            const res = (await addStudy(data.studyName, data.piName, data.studyNumber, data.studyKeywords, data.studyInitDate, filesBase64)) as {
                message: string;
            };
    ...
    useEffect(() => {
        if (file) {
            const arrayFile = Object.values(file).flat().filter(item => item !== undefined);
            const promises = arrayFile.map(item => new Promise<string>((resolve) => {
    ...
    return (
        <div className="w-full h-[calc(100%-2rem)] flex flex-col justify-between ">
            ...
            <Button
                ...
                disabled={!isValid || file?.IRB === undefined || file.protocol === undefined || file.summary === undefined}
            >
